 ...
 *
 * Git passes push data via stdin to the pre-push hook. We capture it into
 * an env var, then reopen stdin from /dev/tty so Node can show interactive
 * prompts to the user in the terminal.
 */
...
# Capture git's push data from stdin.
GATEKEEPER_PUSH_DATA=$(cat)

# Try to reopen stdin from the terminal so Node can show interactive prompts.
# If /dev/tty is unavailable (CI, IDE subprocess), mark as non-interactive.
if [ -r /dev/tty ]; then
  exec < /dev/tty
else
  GATEKEEPER_NON_INTERACTIVE=1
fi

GATEKEEPER_PUSH_DATA="$GATEKEEPER_PUSH_DATA" GATEKEEPER_NON_INTERACTIVE="$GATEKEEPER_NON_INTERACTIVE" node "${hookJsPath}"
exit $?
`;
}
